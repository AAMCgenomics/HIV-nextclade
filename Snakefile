...
# Subtipos para incluir o excluir
...
rule make_metadata:
    ...
    run:
        ...
        # Corrección: Se eliminó la clave 'country' duplicada
        fields = {'strain': -1, 'subtype': 0, 'date': 2, 'accession': -1,  'name': [3, -1], 'country': 1}
...
rule align:
    ...
    params:
        # Corrección: Se simplificó 'params' para pasar la plantilla a nextclade
        translations_pattern="results/translation_{gene}.fasta"
    threads: 4
    shell:
        """
        nextclade3 run -j {threads} --input-ref {input.ref} \
                       --input-pathogen-json {input.pjson} --input-annotation {input.annotation} \
                       --output-fasta {output.alignment} --output-translations {params.translations_pattern} \
...
rule split_by_subtype:
    ...
    run:
        ...
        import os
        from Bio import SeqIO
        os.makedirs(output.alignments, exist_ok=True)
...
rule ancestral:
    ...
    params:
        # Corrección: Se crearon cadenas con los flags repetidos para augur
        genes = " ".join(genes),
        translations_str = lambda wildcards, input: " ".join([f"--translations {f}" for f in input.translations])
    shell:
        """
        augur ancestral --tree {input.tree} --alignment {input.aln} \
                        {params.translations_str} --genes {params.genes}\
...
rule add_metadata:
    ...
    run:
        ...
        for node in T.get_terminals():
            node_meta = metadata.get(node.name)
            if node_meta:
                nodes[node.name] = {k1:node_meta.get(k2) for k1,k2 in
...
rule export:
    ...
    output:
        "results/tree.json"
    params:
        # Corrección: Se creó una cadena con --node-data repetido para cada archivo
        node_data_str = lambda wildcards, input: " ".join([f"--node-data {f}" for f in input.node_data])
    shell:
        """
        augur export v2 --tree {input.tree} {params.node_data_str} --metadata {input.metadata} \
...
# --- Reglas para empaquetar y validar (Corregidas para evitar dependencias circulares) ---

rule assemble:
    """
    Corrección: Esta regla ahora empaqueta los resultados finales.
    Se ejecuta al final del flujo principal.
    """
    input:
        ref="config/reference.fasta",
        gff="config/genome_annotation.gff3",
        pathogen="config/pathogen.json",
        changelog="config/CHANGELOG.md",
        readme="config/README.md",
        tree="results/tree.json",
        sequences="results/example_sequences.fasta"
    output:
        out_dir = directory("dataset"),
        final_tree = "dataset/tree.json",
        final_seqs = "dataset/sequences.fasta"
    shell:
        """
        mkdir -p {output.out_dir}
        cp {input.ref} {input.gff} {input.pathogen} {input.changelog} {input.readme} {output.out_dir}
        cp {input.tree} {output.final_tree}
        cp {input.sequences} {output.final_seqs}
        """
...
rule validate:
    ...
    shell:
        """
        nextclade run --input-dataset dataset {input.sequences} --output-tsv {output.res}
